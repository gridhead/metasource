...
func TestOthr_Init_Success_Release(t *testing.T) {
	Path_Init(t, "")

	router := option.Navigate()
	rqster := httptest.NewRequest("GET", "/rawhide/changelog/systemd", nil)
	...
	rslt := dict.UnitOther{}
	expt := json.Unmarshal([]byte(record.Body.String()), &rslt)
	if expt != nil {
...
func TestOthr_Init_Success_Updates(t *testing.T) {
	Path_Init(t, "updates")
...
func TestOthr_Init_Failure_AbsentVers_C400(t *testing.T) {
	Path_UnInit(t, "/var/tmp")

	router := option.Navigate()
	rqster := httptest.NewRequest("GET", "//changelog/systemd", nil)
	record := httptest.NewRecorder()
	router.ServeHTTP(record, rqster)
...
func TestOthr_Init_Failure_ReadPrmy_Misc_C400(t *testing.T) {
	Path_UnInit(t, "/var/tmp")
...
func TestOthr_Init_Failure_ReadPrmy_Lost_C404(t *testing.T) {
	Path_UnInit(t, "/var/tmp")
...
func TestOthr_Init_Failure_ReadOthr_Misc_C400(t *testing.T) {
	Path_Init(t, "")

	original := lookup.ReadOthr
	lookup.ReadOthr = func(vers *string, pack *home.PackUnit, repo *string) (home.OthrRslt, error) {
		return home.OthrRslt{}, errors.New("ReadOthr failed")
	}
	defer func() { lookup.ReadOthr = original }()
...
func TestOthr_Init_Failure_AbsentName_C404(t *testing.T) {
	Path_UnInit(t, "/var/tmp")

	router := option.Navigate()
	rqster := httptest.NewRequest("GET", "/rawhide/changelog/", nil)
	record := httptest.NewRecorder()
	router.ServeHTTP(record, rqster)
