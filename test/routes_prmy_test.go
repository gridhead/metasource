...
func TestPrmy_Init_Success(t *testing.T) {
	Path_Init(t, "")

	router := option.Navigate()
	rqster := httptest.NewRequest("GET", "/rawhide/pkg/systemd", nil)
...
func TestPrmy_Init_Success_Updates(t *testing.T) {
	Path_Init(t, "updates")
...
func TestPrmy_Init_Failure_AbsentVers_C400(t *testing.T) {
	Path_UnInit(t, "/var/tmp")

	router := option.Navigate()
	rqster := httptest.NewRequest("GET", "//pkg/systemd", nil)
	record := httptest.NewRecorder()
	router.ServeHTTP(record, rqster)
...
func TestPrmy_Init_Failure_ReadPrmy_Misc_C400(t *testing.T) {
	Path_UnInit(t, "/var/tmp")
...
func TestPrmy_Init_Failure_ReadPrmy_Lost_C404(t *testing.T) {
	Path_UnInit(t, "/var/tmp")
...
func TestPrmy_Init_Failure_ReadExtn_Misc_C400(t *testing.T) {
	Path_Init(t, "")
...
func TestPrmy_Init_Failure_ReadCoop_Misc_C400(t *testing.T) {
	Path_Init(t, "")
...
func TestPrmy_Init_Failure_AbsentName_C404(t *testing.T) {
	Path_UnInit(t, "/var/tmp")

	router := option.Navigate()
	rqster := httptest.NewRequest("GET", "/rawhide/pkg/", nil)
	record := httptest.NewRecorder()
	router.ServeHTTP(record, rqster)
